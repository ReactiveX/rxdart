...
class GroupByObservable<T, S> extends StreamObservable<GroupByMap<S, T>> {
  ...
  GroupByObservable(Stream<T> stream, S keySelector(T value), {int compareKeys(S keyA, S keyB): null}) {
    ...
    setStream(stream.transform(new StreamTransformer<T, GroupByMap<S, T>>(
          ...
          controller = new StreamController<GroupByMap<S, T>>(sync: true,
              onListen: () {
                subscription = input.listen((T value) {
                  bool doAdd = false;
                  S key = keySelector(value);
                  GroupByMap<S, T> map = allMaps.firstWhere((GroupByMap<S, T> existingMap) => compareKeys(existingMap.key, key) == 0, orElse: () {
...
class GroupByMap<T, S> {
  ...
  Observable<S> get observable => new StreamObservable<S>()..setStream(_controller.stream);
