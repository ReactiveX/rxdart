...
- `isSuspendedAtYield == false` → generator is **not** resumed.
  `cancellationFuture` is returned but **nothing will complete it**. ❌

### `close()` — the **only** way `cancellationFuture` completes

```dart
close() {
  final future = cancellationFuture;
  if ((future != null) && future._mayComplete) {
    future._completeWithValue(null);   // ← the ONLY completion path
  }
  controller.close();
}
```
...
because `close()` is only called when the `async*` body returns.
