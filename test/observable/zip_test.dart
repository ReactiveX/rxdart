...
void main() {
  test('rx.Observable.zip', () async {
    ...
    Stream<List<dynamic>> observable = rx.Observable.zipThree(
        new Stream<int>.periodic(const Duration(milliseconds: 20), (int count) => count).take(4),
  ...
  test('rx.Observable.zip.asBroadcastStream', () async {
    ...
    Stream<List<dynamic>> observable = rx.Observable.zipThree(
        new Stream<int>.periodic(const Duration(milliseconds: 20), (int count) => count).take(4),
        new Stream<int>.fromIterable(const <int>[1, 2, 3, 4, 5, 6, 7, 8, 9]),
        testStream.stream, (int a, int b, bool c) => <dynamic>[a, b, c], asBroadcastStream: true);
