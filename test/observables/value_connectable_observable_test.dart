...
void main() {
  group('BehaviorConnectableObservable', () {
    ...
    test('should begin emitting items after connection', () {
      var count = 0;
      const items = [1, 2, 3];
      final observable = ValueConnectableStream(Stream.fromIterable(items));
    ...
    test('stops emitting after the connection is cancelled', () async {
      final observable = Stream.fromIterable(const [1, 2, 3]).publishValue();
    ...
    test('multicasts a single-subscription stream', () async {
      final observable = ValueConnectableStream(
