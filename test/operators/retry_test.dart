...
void main() {
  test('rx.Observable.retry', () async {
    rx.observable(_getStream()).retry(3).listen(expectAsync1((int result) {
          expect(result, 3);
        }, count: 1));
  });
  ...
  test('rx.Observable.retry.asBroadcastStream', () async {
    Stream<int> observable =
        rx.observable(_getStream().asBroadcastStream()).retry(3);
  ...
  test('rx.Observable.retry.error.shouldThrow', () async {
    Stream<int> observableWithError = rx.observable(_getStream()).retry(2);
